...
%%
% I had trouble actually being able to implement the filtfilt command
% because the dimensions of the signal matrix said they were not compatible
% the with dimensions required to use the command (signal is 120 samples, 
% requires 156 length vector, and I tried using a zero buffer but it failed
% (horribly), but from my understanding of what the filter does, using the
% filtfilt command runs the filter across the signal twice, attenuating a 
% signal by the amplitude of the filter squared (or doubled in dB).